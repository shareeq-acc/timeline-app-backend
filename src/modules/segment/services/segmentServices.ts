...
import { CreateSegmentRequestDto, UpdateSegmentRequestDto, SegmentResponseDto, GetSegmentsResponseDto, CreateBulkSegmentsRequestDto, SegmentExtendedDto } from '../types/dtos';
...
import { SegmentScheduling } from '../models/SegmentScheduling';
...
export class SegmentService {
    ...
    async createSegment(
        ...
        try {
            ...
            return Segment.composeSegment(segment, true);
        } catch (error) {
    ...
    async getSegmentById(id: string, userId: string): Promise<SegmentExtendedDto> {
        try {
            ...
            return Segment.composeSegment(segment, isAuthor);
        } catch (error) {
    ...
    async getSegmentsByTimelineId(
        ...
        try {
            ...
            const isAuthor:boolean = timeline.author.id === userId;
            const { segments, total } = await Segment.findByTimelineId(timelineId, page, limit);

            const enhancedSegments = []
            for(let i=0; i<segments.length; i++){
                const enhancedSegment = Segment.composeSegment(segments[i], isAuthor);
    ...
    async updateSegment(
        ...
        try {
            ...
            const updatedSegment = await Segment.update(id, data);
            if (!updatedSegment) {
                throw new AppError(
            ...
            return Segment.composeSegment(updatedSegment, true);
        } catch (error) {
    ...
    async deleteSegment(id: string, userId: string): Promise<void> {
        try {
            ...
            const deleted = await Segment.delete(id);
            if (!deleted) {
                throw new AppError(
                    ERROR_CODES.INTERNAL_SERVER_ERROR.httpStatus,
                    ERROR_CODES.INTERNAL_SERVER_ERROR.code,
    ...
    async createBulkSegments(
        ...
        try {
            ...
            const segments = await Segment.createBulk(data);
            let detailedSegments = [];
            for(const segment of segments) {
                const currentSegment = Segment.composeSegment(segment, true);
    ...
    async markSegmentComplete(id: string, userId: string): Promise<SegmentExtendedDto> {
        try {
            ...
            const segmentSch = await SegmentScheduling.findBySegmentId(id);
            ...
            const completedSegment = await SegmentScheduling.markComplete(id);
            if (!completedSegment) {
                throw new AppError(
                    ERROR_CODES.INTERNAL_SERVER_ERROR.httpStatus,
                    ERROR_CODES.INTERNAL_SERVER_ERROR.code,
    ...
    async updateSegmentScheduleDate(
        ...
        try {
            ...
            if(unitNumber != 1){
                ...
                const previousSegment = await Segment.findByUnitNumber(segment.timelineId, unitNumber-1);
                if(!previousSegment){
                    throw new AppError(
                        ERROR_CODES.NOT_FOUND.httpStatus,   
                        ERROR_CODES.NOT_FOUND.code,
                ...
                const previousSegmentSchedule = await SegmentScheduling.findBySegmentId(previousSegment.id);
            ...
            const currentTime = new Date().toISOString();
            return await SegmentScheduling.updateScheduleDate(id, currentTime);
